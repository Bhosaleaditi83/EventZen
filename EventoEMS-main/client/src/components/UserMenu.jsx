...
const UserMenu = ({close}) => {
    ...
    return (
        <div>
            ...
            {
              isAdmin(user.role) && (
                <Link onClick={handleClose} to={"/dashboard/eventscategory"} className='px-2 hover:bg-orange-200 py-1'>Events Category</Link>
              )
            }
            ...
            {
              isAdmin(user.role) && (
                <Link onClick={handleClose} to={"/dashboard/createevents"} className='px-2 hover:bg-orange-200 py-1'> Upload Events</Link>
              )
            }

            {
              isAdmin(user.role) && (
                <Link onClick={handleClose} to={"/dashboard/events"} className='px-2 hover:bg-orange-200 py-1'>Events</Link>
              )
            }

            <Link onClick={handleClose} to={"/dashboard/mybookings"} className='px-2 hover:bg-orange-200 py-1'>Event Bookings</Link>
