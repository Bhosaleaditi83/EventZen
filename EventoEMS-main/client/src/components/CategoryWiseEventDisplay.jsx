...
const CategoryWiseEventDisplay = ({ id, name }) => {
  ...
  const handleRedirectProductListpage = ()=>{
      const subcategory = subCategoryData.find(sub =>{
        const filterData = sub.eventscategory.some(c => {
          return c._id == id
        })
                ...
                <div className=' flex gap-4 md:gap-6 lg:gap-8 container mx-auto px-4 overflow-x-scroll scrollbar-none scroll-smooth' ref={containerRef}>
                    {loading &&
                        loadingCardNumber.map((_, index) => {
                            return (
                                <CardLoading key={"CategorywiseEventDisplay123" + index} />
                            )
                        })
                    }
...

                    {
                        data.map((p, index) => {
                            return (
                                <CardEvent
                                    data={p}
                                    key={p._id + "CategorywiseEventDisplay" + index}
                                />
                            )
                        })
                    }
...
export default CategoryWiseEventDisplay
